...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    const ImU32 col = (held && hovered) ? ImColor(90, 90, 90) : hovered ? ImColor(70, 70, 70) : ImColor(80,80,80);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, 3);

    window->DrawList->AddText(bb.Min + size_arg / 2 - ImGui::CalcTextSize(label) / 2, ImColor(230,230,230), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImU32 frame_col = hovered ? ImColor(70, 70, 70) : ImColor(80, 80, 80);
    const float value_x2 = ImMax(frame_bb.Min.x, frame_bb.Max.x - arrow_size);
    RenderNavHighlight(frame_bb, id);
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max.x, frame_bb.Max.y), frame_col, 3, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(window->DrawList, ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, GetColorU32(col_rgb), grid_step, ImVec2(-grid_step + off, off), 4, ImDrawCornerFlags_TopRight | ImDrawCornerFlags_BotRight);
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha), 4, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotLeft);
    }
    else
    {
        ...
        if (col_source.w < 1.0f)
            ...
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 4, ImDrawCornerFlags_All);
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(90, 90, 90), 4);
        else
            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(90, 90, 90), 4); // Color button are often in need of some sort of border
    }
