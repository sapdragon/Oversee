...
private:
	...
	constexpr static WireType kWireTypeForFieldType[/*MAX_FIELD_TYPE*/18 + 1] = {
...
Field Field::ReadField(void* data, size_t& bytesRead)
{
	...
	case WIRETYPE_START_GROUP:
		throw(_S("WIRETYPE_START_GROUP unrealised"));
		break;
	case WIRETYPE_END_GROUP:
		throw(_S("WIRETYPE_END_GROUP unrealised"));
		break;
...
ProtoWriter::ProtoWriter(void* data, size_t size, size_t maxFields) : ProtoWriter(maxFields)
{
	...
	while (pos < size) {
		...
		if (index >= vector_size) throw(_S("fields range error: field[%i]"), index);
...
void ProtoWriter::print()
{
	...
	for (size_t i = 0; i <= size; ++i) {
		printf(_S("%.2X "), *(unsigned char*)((uintptr_t)mem + i));
		j++;
		if (j == 16)
		{
			j = 0;
