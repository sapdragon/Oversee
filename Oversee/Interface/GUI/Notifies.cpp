...
namespace notifies
{
	...
	void show_block(ImDrawList* draw_list, const notify_data_t& notify)
	{
		...
		ImGui::GetOverlayDrawList()->AddRectFilled(block_pos, notify_end_pos, alpha_modulate(background_color, alpha), 2.f);
		...
		ImGui::GetOverlayDrawList()->AddRectFilled(block_pos, ImVec2(block_pos.x, block_pos.y + notify_size.y), alpha_modulate(color, alpha), 1.f);
		...
		ImGui::GetOverlayDrawList()->AddText(text_pos, alpha_modulate(text_color, alpha), text_size.x > notify_size.x + 10.f ? notify.message.substr(0, 25).c_str() : notify.message.c_str());
